...
setwd( "D:/Analytics/R/Dataset/Predictive Modelling")
#Loan<-read.csv(file.choose(), header=T)
Loan<-read.csv("default.csv", header=T)
na.omit(Loan)
...
#  For Loan.scatter, consider numeric variables alone as categorical variables are already 
#  included as saperate columns
...
summary(OLS.norm)
###But there are no differences in Rsquare values after normalization 
